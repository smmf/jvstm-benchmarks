...
public class Benchmark {
	...
	public static void main(String[] args) {
		...
		/* disabled to print only commit stats */
 		...
 		benchmark.showStats();
	//	if (Parameters.synchronizationType == Parameters.SynchronizationType.STM) {
	//	    jvstm.CommitStats.printAllStats();
	//	}
		//System.out.println("VBOX = " + jvstm.VBox.counter.get());
	}
	...
	private Benchmark(String[] args, boolean reexecution) throws BenchmarkParametersException {
	...
	private void printRunTimeParametersInformation() {
		...
		System.out.println("Number of threads: " + Parameters.numThreads + "\n" +
				...
				"Syncronization method: " + Parameters.synchronizationType + "\n" +
	...
	private void generateOperationCDF() {
		...
		double rwTraversalsRatio = (Parameters.longReadWriteTraversalsEnabled 
	...
	private void setupStructures() {
		if (Parameters.synchronizationType == Parameters.SynchronizationType.STM) {
		    jvstm.Transaction.beginUnsafeSingleThreaded();
		}
		System.err.println("Setup start...");
		setup = new Setup();

		threads = new BenchThread[Parameters.numThreads];
		
		createOperations(setup);
		
		for(short threadNum = 0; threadNum < Parameters.numThreads; threadNum++) {
			threads[threadNum] = 
				new BenchThread(setup, operationCDF, threadNum, operations);
		}
		...
		for(OperationId operationDescr : OperationId.values()) {
			...
			try {
				Constructor<? extends Operation> operationConstructor = 
					operationClass.getConstructor(setup.getClass());
	...
	private void checkInvariants(boolean initial) {
	...
	private void createInitialClone() {
	...
	private void start() {
		...
		try {
			...
			for(BenchThread thread : threads) thread.stopThread();
	...
	private void showStats() {
		...
		if (Parameters.synchronizationType == Parameters.SynchronizationType.STM) {
			...
			stmbench7.impl.jvstm.JVSTMStats.printStats();
			//smf:// jvstm.TxStats.printStats();
		}
	}
